...
for i in $(seq 0 $TRANSIT_MAX); do
    for j in $(seq 0 $EDGE_MAX); do
        create_vxlan_tunnel "ed${VPC_ID}$j" "$TRANSIT_ID" "t${TRANSIT_ID}${i}0" "2.$VPC_ID.$j.1" "transit${TRANSIT_ID}-${i}" "${VPC_ID}${i}${j}"
    done
    # TODO remove this 
    # ip netns exec "tr${TRANSIT_ID}0" ip route add 1.0.0.0/8 dev "vbr-t$TRANSIT_ID"
done
