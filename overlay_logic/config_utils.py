...
class config_utils:
    def __init__ (self) :
        self.BGP_CONFIG_DIRECTORY = os.path.dirname(os.path.realpath(__file__)).replace("overlay_logic", "overlay_south") + "/bgp/ready"
    ...
    def write_config_to(self, filename):
        ...
        fd = open(self.BGP_CONFIG_DIRECTORY+"/"+filename, 'w')
        fd.write(config)
        fd.close()
