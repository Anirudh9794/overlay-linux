...
import ipaddress
import traceback
from overlay_logic.vpc_handler import bgp_vpc_map
        ...
        if(self.db.get_subnet(ip_address, vpc_id) != None):
            ...
            if self.create_physically(ip_address, vpc_id, vpc.host, vpc.scaleFactor):
                ...
                for i in vpc.scaleFactor:
                    ns = "V%sE%s" % (vpc_id, i)
<<<<<<< HEAD
                    self.restart_quagga_service(vpc.host.lower(), vpc_id, i)
=======
                    self.restart_quagga_service(vpc.host.lower(), ns)
>>>>>>> cdd23211b138f9a64d26d1d67a7696826a9258e6
    ...
    def restart_quagga_service(self, host, vpc_id, edge_number):
        result = False
        host = host.lower()

        try:
            json_obj = {}
            json_obj["edge_number"] = edge_number
            ...
            if p.returncode != 0:
                # print(
                    # "\r\nUnable to run ansible 'restart-bgp-container.yml' for subnet '{0}' in VPC '{1}'\r\n".format(subnet_id, vpc_id))
                print("error in restarting container")
                return False

            result = True
        except Exception as e:
            # print(
                # "\r\nUnable to physically restart container '{0}' for vpc id '{1}'\r\nException below:\r\n{2}".format(subnet_id, vpc_id, e))
            print("error in restarting container")
            result = False
